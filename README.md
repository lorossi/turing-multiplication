...
Other than adding two numbers, an easy task that can almost be done via an *FSA*, or some "classic" computing examples such as accepting string generated by contextual grammars (the ones generated by the language $L = \left\lbrace a^n b^n c^n \mid n \geq 1\right\rbrace$), there was nothing really interesting or complex. In the course, we explored many algorithms, but most of the exercises and exams asked *(luckily!)* only to implement natural language-related tasks, such as accepting strings, via Turing Machines.
...
- An **output function** $\eta$
  - $\eta: (Q-F) \times I \times \Gamma^k \rightarrow O \times \left\lbrace\rightarrow, \leftarrow, \downarrow\right\rbrace$
